...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    // +1 to include the null byte that snprintf wrote
    ...
    if (object_exists(id_out)) {
        ...
        printf("Object stored at: %s/%.2s/%s\n", OBJECTS_DIR, hex, hex + 2);
        free(full);
        return 0;
    }
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    // Step 6: Extract data portion (everything after the null byte)
    size_t data_offset = (size_t)(null_pos - buf) + 1;
    size_t data_size = (size_t)file_size - data_offset;

    *len_out = data_size;
    *data_out = malloc(data_size + 1); // +1 for safe null termination
    if (!*data_out) { free(buf); return -1; }
    memcpy(*data_out, buf + data_offset, data_size);
